...
class NNUE(pl.LightningModule):
  ...
  def __init__(self, feature_set, start_lambda=1.0, end_lambda=1.0, max_epoch=800, gamma=0.992, lr=8.75e-4, param_index=0, num_psqt_buckets=8, num_ls_buckets=8):
    ...
    self._init_layers()
    self.possible_forward_modes = [1,2,3,4]
  ...
  def forward(self, us, them, white_indices, white_values, black_indices, black_values, psqt_indices, layer_stack_indices):
    ...
    if self.forward_mode in self.possible_forward_modes:
        return self.layer_stacks(l0_, layer_stack_indices, forward_mode = self.forward_mode)
    else:
      x = self.layer_stacks(l0_, layer_stack_indices, forward_mode = self.forward_mode) + (wpsqt - bpsqt) * (us - 0.5)
